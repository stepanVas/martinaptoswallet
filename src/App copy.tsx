...
function App() {
  ...
  const [address, setAddress] = React.useState<string | null>(null); //получаем адрес кошелька
/*   const [account, setAccount] = React.useState<Types.AccountData | null>(null);//сведения об аккаунте
  React.useEffect(() => { 
    if (!address) return;
  ...
  const init = async() => {
    ...
    const { response, publicKey } = await window.martian.connect();
